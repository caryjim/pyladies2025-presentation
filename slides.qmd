...
-   ( n~0~ ) = initial sample size from Cochran’s formula for an infinite population
...
-   ( n~0~ ) = theoretical sample value obtained from Cochran’s formula for an infinite population
-   ( N ) = population size
...
::: notes
In this slide, we’re introducing Cochran’s Sample Size Formula, a widely used method in educational and survey research to determine how many participants are needed for statistically valid results. We start with the ideal sample size formula for an infinite population. This formula uses three key parameters:

Z-value, which corresponds to the desired confidence level (e.g., 1.96 for 95% confidence), p, the estimated proportion of the population with the attribute of interest (we often use 0.5 to account for maximum variability), and e, the margin of error we’re willing to accept (commonly 0.05 for 5%).

Once we calculate the ideal sample size, we adjust it for a finite population—in our case, 25,000 college students—using a modified formula. This ensures our sample size is appropriate for the actual population size. The Python code example demonstrates how to implement this calculation programmatically. It uses the scipy.stats library to compute the Z-value and then applies both formulas to get the ideal and adjusted sample sizes. This process helps ensure that our survey results are both accurate and representative of the population we’re studying.
:::

## Re-code Age to Age Group {.smaller}

**2.3 Code example: Grouping age by increments of 2 or 10 {.smaller}**
...
::: notes
In this step, we re-code the age variable into categorical age groups using pandas' `cut` function and we used the increments of 2. However, this grouping is a bit problematic because the age are skewed to the younger age groups. We have to re-do the categorization to make it more useful for stratified sampling.

Now. You may be wondering why, and the reason is when we are using the stratified method, each level within a group is a part of a combination. For example, if I have 2 levels for Gender, and then 5 levels for Race, I'm really looking at 10 different ways of the combination between gender and race. If your finite population is not big enough, technically, you will not be able to stratified meaningfully to account for all the different combination and have a decent sample size group to survey.
:::

**2.4 Code example: Grouping age by intended age range {.smaller}**

```{python}
# In previous conversion, any groups beyond the age of 40 is really small, and the population size for age 23 or above is much smaller by proportion to the younger age groups.

# Therefore, we can further combine age 23 to 30 into one group, and then combine age 31 and above into a single category, called '31+'.

# Ensure Age_Group is categorical and handle NaNs
...
# Add any missing categories to avoid ValueError
missing_cats = set(new_categories.keys()) - set(population_data['Age_Group'].cat.categories)
if missing_cats:
    population_data['Age_Group'] = population_data['Age_Group'].cat.add_categories(list(missing_cats))

population_data['Age_Group'] = population_data['Age_Group'].replace(new_categories)
population_data['Age_Group'].value_counts().sort_index()
```

This time the levels in the age group is more appropriate and have a decent count at each level. Remember we still need to use this information to calculate proportion and so in practice, you may repeat this steps and the sampling part until you are able to draw a proper set of subgroups as a final complete sample.
...
## 3.1 Code example: Caculate proportions and apply it to select stratum {.smaller .scrollable} 
...
sampled_stratum_counts.head(10)
```
...
Invitations Required=20000\* 10=20,000
